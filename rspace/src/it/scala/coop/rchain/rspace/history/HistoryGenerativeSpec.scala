...
class HistoryGenerativeSpec
  ...
  "process" should "accept new leafs (insert, update, delete)" in forAll(
    ...
    val emptyMergingHistory =
      HistoryInstances.merging[Task](HistoryInstances.emptyRootHash, inMemHistoryStore)

    val emptySimplisticHistory: History[Task] =
      SimplisticHistory.noMerging[Task](HistoryInstances.emptyRootHash, inMemHistoryStore)
    ...
    runEffect(finalMergingHistory).root shouldBe HistoryInstances.emptyRootHash

    val finalSimplisticHistory = resultSimplisticHistory.process(deletions)
    runEffect(finalSimplisticHistory).root shouldBe HistoryInstances.emptyRootHash
  }
  ...
  "process" should "accept new leafs in bulk" in forAll(distinctListOf(arbitraryRandomThreeBytes)) {
    keys: List[Key] =>
      ...
      postDeletionNonMergingHistory.root shouldBe HistoryInstances.emptyRootHash
  }
