...
import io.rhonix.models.{GPrivate, GUnforgeable, Par}
...
class TreeHashMapMergeabilitySpec
  ...
  it should "read on the same treeHashMap is mergeable" in forAll(
    listKeyValuesGen,
    // rholang treeHashMap depth range based on https://github.com/rhonixlabs/rhonix/blob/dev/casper/src/main/resources/Registry.rho#L60
    Gen.chooseNum(0, 16),
    minSuccessful(2)
  ) { (initialKVS, depth) =>
    TreeHashMapMergeableTestCase(
