...
import io.rhonix.casper.protocol.client.{DeployRuntime, GrpcDeployService, GrpcProposeService}
import io.rhonix.crypto.PrivateKey
import io.rhonix.crypto.signatures.{Secp256k1, SignaturesAlg}
import io.rhonix.crypto.util.KeyUtil
import io.rhonix.models.syntax._
import io.rhonix.monix.Monixable
import io.rhonix.node.configuration.Configuration.Profile
import io.rhonix.node.configuration._
import io.rhonix.node.effects
import io.rhonix.node.effects.{ConsoleIO, GrpcReplClient}
import io.rhonix.node.web.VersionInfo
import io.rhonix.shared.StringOps.StringColors
import io.rhonix.shared.{Log, TerminalMode, Time}
...
object NodeMain {
  ...
  private def checkPorts[F[_]: Sync: Log](conf: NodeConf): F[NodeConf] = {
    ...
    def checkRhonixProtocolPort(configuration: NodeConf): F[NodeConf] =
      isLocalPortAvailable(configuration.protocolServer.port).ifM(
        ...
        if (configuration.protocolServer.useRandomPorts)
          for {
            port <- getFreePort
            _    <- Log[F].info(s"Using random port $port as Rhonix Protocol port")
          } yield configuration.copy(
            protocolServer = configuration.protocolServer.copy(port = port)
          )
        else
          Log[F].error(
            "Hint: Run me with --use-random-ports to use a random port for Rhonix Protocol port"
          ) >> Sync[F].raiseError(new Exception("Invalid Rhonix Protocol port"))
      )
    ...
    for {
      ...
      rpPortConf       <- checkRhonixProtocolPort(conf)
