package io.rhonix.node.configuration.commandline

import java.nio.file.{Path, Paths}

import io.rhonix.casper.protocol.client.ListenAtName.{Name, PrivName, PubName}
import io.rhonix.comm.PeerNode
import io.rhonix.crypto.signatures.Ed25519
  ...
  val run = new Subcommand(
    ...
    descr(
      "Start RNode server. For defaults see " +
        "https://github.com/rhonixlabs/rhonix/blob/dev/node/src/main/resources/reference.conf\n"
    )
    helpWidth(width)
    ...
    val networkId = opt[String](
      descr = "ID of the Rhonix network to connect to."
    )
    ...
    val host = opt[String](
      descr = "Address to bind Rhonix Protocol server."
    )

    val useRandomPorts = opt[Boolean](
      descr = "Use random ports in case Rhonix Protocol port and/or Kademlia port are not free."
    )
    ...
    val protocolPort = opt[Int](
      short = 'p',
      descr = s"gRPC port serving Rhonix Protocol messages. Defaults to 40404"
    )
