...
import io.rhonix.casper.protocol.{BlockInfo, DataWithBlockInfo, DeployData, LightBlockInfo}
...
import io.rhonix.models.rholang.implicits.VectorPar
import io.rhonix.models.syntax._
import io.rhonix.models.{Bundle, Expr, GUnforgeable, Par}
import io.rhonix.node.api.WebApi._
import io.rhonix.node.web.{CacheTransactionAPI, TransactionResponse}
...
object WebApi {
  ...
  def toApiStatus(status: io.rhonix.casper.protocol.Status) =
    ApiStatus(
  ...
  private def toDeployExecStatus(
      status: io.rhonix.casper.protocol.deploy.v1.DeployExecStatus
  ): Option[DeployExecStatus] = {
    import io.rhonix.casper.protocol.deploy.v1.DeployExecStatus.{Status => DepSt}
    status.status match {
