...
import fs2.{io => fsio, text, Pipe, Stream}
...
object BondsParser {
  ...
  private def newValidators[F[_]: Sync: ContextShift: Log](
    ...
    def toFile(filePath: Path, blocker: Blocker): Pipe[F, String, Unit] =
      _.through(text.utf8Encode).through(fsio.file.writeAll(filePath, blocker))
    ...
    Blocker[F].use { blocker =>
      fsio.file.createDirectories(blocker, genesisFolder) *>
