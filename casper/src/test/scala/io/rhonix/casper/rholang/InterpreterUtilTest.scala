...
import io.rhonix.casper.MultiParentCasper
import io.rhonix.casper.helper._
import io.rhonix.casper.protocol._
import io.rhonix.casper.rholang.InterpreterUtil._
import io.rhonix.casper.rholang.Resources.mkRuntimeManager
import io.rhonix.casper.rholang.RuntimeManager.StateHash
import io.rhonix.casper.rholang.types.SystemDeploy
import io.rhonix.casper.util.GenesisBuilder.{buildGenesis, buildGenesisParametersSize}
import io.rhonix.casper.util.RSpaceUtil._
...
import io.rhonix.p2p.EffectsTestInstances.LogStub
...
class InterpreterUtilTest
  ...
  "used deploy with insufficient phlos" should "be added to a block with all phlos consumed" in effectTest {
    val sampleTerm =
      """
        |  new
        |    rl(`rho:registry:lookup`), RevVaultCh, vaultCh, balanceCh, deployId(`rho:rhonix:deployId`)
        |  in {
  ...
  val multiBranchSampleTermWithError =
    """
      ...
      |      contract signal(@x) = {
      |        rl!(`rho:rhonix:revVault`, *RevVaultCh) | ackCh!(x) |
