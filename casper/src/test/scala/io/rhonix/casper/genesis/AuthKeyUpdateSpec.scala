...
import io.rhonix.models.rholang.RhoType.{RhoBoolean, RhoString, RhoTuple2}
...
class AuthKeyUpdateSpec extends AnyFlatSpec with Matchers with Inspectors {
  ...
  "deploy with correct private key" should "update the rho:rhonix:authKey right" in effectTest {
    ...
    val exploreUpdateResultTerm =
      """new return, registryLookup(`rho:registry:lookup`), resCh, ret in {
                        #  registryLookup!(`rho:rhonix:authKey`, *resCh) |
