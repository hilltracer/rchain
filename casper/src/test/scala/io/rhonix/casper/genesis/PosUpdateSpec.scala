...
import io.rhonix.models.GDeployId
import io.rhonix.models.rholang.RhoType.{RhoBoolean, RhoNumber, RhoString, RhoTuple2}
...
class PosUpdateSpec extends AnyFlatSpec with Matchers with Inspectors {
  ...
  "deploy with correct private key" should "update the rho:rhonix:pos right" in effectTest {
    val updateDeploy =
    ...
    val transferTerm =
      s"""
         #new rl(`rho:registry:lookup`), RevVaultCh, vaultCh, toVaultCh, deployerId(`rho:rhonix:deployerId`), stdout(`rho:io:stdout`),revVaultKeyCh, resultCh in {
    ...
    val exploreUpdateResultTerm =
      """new return, registryLookup(`rho:registry:lookup`),stdout(`rho:io:stdout`), resCh, ret in {
        #  registryLookup!(`rho:rhonix:pos`, *resCh) |
        #  for (@(nonce, pos) <- resCh){
        #    stdout!((pos, "get pos"))|
