...
object NormalizerEnv {
  ...
  def withDeployerId(deployerPk: PublicKey) =
    new NormalizerEnv(
      ("rho:rhonix:deployerId" ->> GDeployerId(ByteString.copyFrom(deployerPk.bytes))) :: HNil
    )

  def apply(deploy: Signed[DeployData]) =
    new NormalizerEnv(
      ("rho:rhonix:deployId" ->> GDeployId(deploy.sig)) ::
        ("rho:rhonix:deployerId" ->> GDeployerId(ByteString.copyFrom(deploy.pk.bytes))) :: HNil
    )

  @implicitNotFound(
    "Elements of ${L} should be Pars. Maybe you forgot to import io.rhonix.models.rholang.implicits._ ?"
  )
  ...
  @implicitNotFound(
    "${Env} must consist of URI strings and Par expressions. Maybe you forgot to import io.rhonix.models.rholang.implicits._ ?"
  )
  trait ToEnvMap[Env] {
