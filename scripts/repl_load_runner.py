...
parser.add_argument("-g", "--grpc-host", dest='grpc_host', default="node0.testnet1.rhonix", help="set grpc host")
...
parser.add_argument("-n", "--network", dest='network', type=str, default="testnet1.rhonix", help="set docker memory for repl client node")
...
cmd = "sudo docker run --rm -it -v {}:/var/lib/rnode --cpus=.5 --memory=1024m --name rnode-repl.{} --network {} io.rhonix/rnode --grpc-host node0.testnet1.rhonix repl".format(volume.name, args.network, args.network)
